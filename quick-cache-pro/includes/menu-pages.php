...
namespace quick_cache // Root namespace.
{
	...
	class menu_pages // Plugin options.
	{
		...
		public function options()
		{
			...
			echo '            <option value="1"'.selected($this->plugin->options['change_notifications_enable'], '1', FALSE).'>'.__('Yes, enable Quick Cache notifications in the Dashboard when changes are detected &amp; one or more cache files are cleared automatically.', $this->plugin->text_domain).'</option>'."\n";
			...
			echo '      <button type="button" class="plugin-menu-page-export-options" style="float:right; margin: 0 0 0 25px;"'. // Exports existing Quick Cache options from this installation.
		...
		public function update_sync()
		{
			...
			echo '      <p>'.__('When a new version of Quick Cache Pro becomes available, WebSharks™ can display a notification in your WordPress Dashboard prompting you to return to this page and perform an upgrade. Would you like this functionality enabled or disabled?', $this->plugin->text_domain).'</p>'."\n";
			echo '      <hr />'."\n";
			...
			echo '            <option value="0"'.selected($this->plugin->options['update_sync_version_check'], '0', FALSE).'>'.__('No, do not display any Quick Cache update notifications in my WordPress Dashboard.', $this->plugin->text_domain).'</option>'."\n";
			echo '         </select></p>'."\n";
			echo '   </div>'."\n";
